...
{
    switch(*(*Line->Instructions)->Bytes)
    {
    case OP_NOP:
...
void cDexCodeGen::GenerateSourceCode()
{
    for (UINT i=0; i<dMethod->LinesSize; i++)
    {
        ...
        {
            for (UINT j=0; j<dMethod->Lines[i]->InstructionsSize; j++)
            {
                switch(*dMethod->Lines[i]->Instructions[j]->Bytes)
                {
                case OP_NOP:
        ...
        if (!strlen(dMethod->Lines[i]->Decompiled))
        {
            ...
            dMethod->Lines[i]->Decompiled = (CHAR*)realloc(dMethod->Lines[i]->Decompiled, strlen(dMethod->Lines[i]->Decompiled)+1);
    }
}
